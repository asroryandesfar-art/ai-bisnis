"""
test_computer_agent_router.py — bn_platform/computer_agent.py: RBAC gating
per route (read/write/approve tier) dan endpoint approve/reject. Mirror pola
test_operations_agent.py/test_workforce_orchestrator.py.
"""
import asyncio
...
from bn_platform.computer_agent import build_computer_agent_router, RejectTaskRequest
...
def test_router_gates_every_route_with_correct_permission_tier():
    ...
    assert requested_keys.count("computer_agent.read") == 2     # list + get
    assert requested_keys.count("computer_agent.approve") == 2  # approve + reject
    assert set(requested_keys) == {"computer_agent.read", "computer_agent.approve"}
...
def _build_router(pool):
    ...
    return build_computer_agent_router(
        get_pool=get_pool, get_current_user=get_current_user,
...
def test_get_task_route_404_when_not_found():
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(handler(
            task_id="t1", user={"org_id": "org-1", "id": "user-1"}, pool=pool,
        ))
    assert exc.value.status_code == 404


def test_approve_task_route_404_when_not_pending_approval():
    pool = FakePool(fetchrow_results=[{"id": "t1", "status": "completed", "plan": "[]"}])
    router = _build_router(pool)
...
def test_reject_task_route_writes_audit_log():
    pool = FakePool(fetchrow_results=[
        {"id": "t1", "status": "pending_approval", "plan": "[]"},
...
def test_list_tasks_route_returns_tasks_key():
    ...
    result = asyncio.run(handler(
        user={"org_id": "org-1", "id": "user-1"}, pool=pool, status=None, limit=50,
    ))
