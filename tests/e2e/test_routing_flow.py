...
def test_chat_routes_to_expected_intent(client, bot, message, expected_intent):
    resp = client.post(f"/chat/{bot}", json={"message": message})
...
def test_explicit_admin_request_allows_handoff_through_real_endpoint(client, bot):
    resp = client.post(f"/chat/{bot}", json={"message": "Saya mau bicara dengan admin"})
    assert resp.status_code == 200, resp.text
    assert resp.json()["handoff_offered"] is True


def test_administrasi_question_does_not_falsely_trigger_handoff(client, bot):
    ...
    resp = client.post(f"/chat/{bot}", json={"message": "Biaya administrasi bulanan berapa ya?"})
    assert resp.status_code == 200, resp.text
