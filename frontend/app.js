...
async function renderObservability(days = state.observabilityDays) {
  ...
  const agentRows = (data.agents || []).map((agent) => {
    ...
    return `<tr><td><span class="table-title mono">${esc(agent.agent_name)}</span></td><td>${formatNumber(agent.executions)}</td><td><span${title}>${statusBadge(kind,label)}</span>${latest==='error'&&agent.last_error?`<div class="subtle" style="font-size:10px;color:#f87171;margin-top:2px;max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(agent.last_error)}</div>`:''}</td><td>${Math.round(agent.average_latency_ms || 0)}ms</td><td>${formatNumber(agent.total_tokens)}</td><td>${agent.last_seen_at ? relativeTime(agent.last_seen_at) : '—'}</td></tr>`;
  }).join("");
