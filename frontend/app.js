...
async function renderBilling() {
  ...
  const planCards = state.plans.map((plan) => {
    ...
    return `<article class="card plan-card ${plan.key===currentKey?'featured':''}"><span class="eyebrow">${plan.key===currentKey?'CURRENT PLAN':'AVAILABLE PLAN'}</span><h3>${esc(plan.name)}</h3><p class="subtle" style="font-size:10px;min-height:32px">${esc(description)}</p>${priceBlock}<ul class="feature-list">${features.slice(0,6).map(feature=>`<li>${esc(String(feature).replace(/_/g,' '))}</li>`).join('')}${extraStats}</ul><button class="button ${plan.key===currentKey?'':'button-primary'}" ${plan.key===currentKey?'disabled':''} data-checkout-plan="${esc(plan.key)}">${buttonLabel}</button></article>`;
  }).join("");
